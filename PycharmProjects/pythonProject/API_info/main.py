...
def save():
    ...
    my_number = my_phone_number_entry.get()
    ...
    if len(api_name) == 0:
        ...
        try:
            ...
            my_phone_number_entry.delete(0, END)
...
def find():
    ...
    try:
        ...
        if api_name in data:
            ...
            phone_number = data[api_name]["phone_number"]
            my_phone_number = data[api_name]["my_phone_number"]
            messagebox.showinfo(title=api_name, message=f"Endpoint: {endpoint}\nAPI ID: {api_id}\nAPI key: {api_key}\nToken: {token}\nPhone number: {phone_number}\nMy phone number: {my_phone_number}")
        else:
...
phone_number_label = Label(text="Phone number:", justify=LEFT).grid(sticky=W, row=6, column=0)
my_phone_number_label = Label(text="My phone number:", justify=LEFT).grid(sticky=W, row=7, column=0)
...
phone_number_entry = Entry(width=60, justify=LEFT)
phone_number_entry.grid(sticky=W, row=6, column=1, columnspan=2)
my_phone_number_entry = Entry(width=60, justify=LEFT)
my_phone_number_entry.grid(sticky=W, row=7, column=1, columnspan=2)
...
add_button.grid(sticky=W, row=8, column=1)
